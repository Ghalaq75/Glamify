...
async function createBooking(req, res) {
  ...
  if (!address || !String(address).trim()) {
    return res.status(400).json({ success: false, message: 'Address is required to confirm a booking.' });
  }

  {
    const coords = await geocodeAddress(address);
    if (coords && !isWithinRiyadh(coords.lat, coords.lng)) {
      return res.status(400).json({ success: false, message: 'Address must be within Riyadh.' });
    }
  }
...
async function createStripeCheckout(req, res) {
  ...
  if (!isStripeConfigured()) {
    return res.status(503).json({ success: false, message: 'Card payments are not configured on this server.' });
  }
  ...
  const origin = (req.body && req.body.returnOrigin)
    || req.headers.origin
    || `${req.protocol}://${req.get('host')}`;
  const successUrl = `${origin}/client/bookings/payment/return?bookingId=${bookingId}&session_id={CHECKOUT_SESSION_ID}`;
  const cancelUrl = `${origin}/client/bookings/payment/return?bookingId=${bookingId}&cancelled=1`;

  const totalSar = Math.max(1, Math.round(Number(booking.totalPrice) || 0));
  const platformFeeSar = Math.max(0, Math.round(Number(booking.platformFee) || 0));
  const grandTotalSar = totalSar + platformFeeSar;
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      line_items: [{
        ...
        price_data: {
          currency: 'sar',
          unit_amount: grandTotalSar * 100,
          product_data: {
            name: `${service ? service.name : 'Glamify booking'} – ${provUser ? provUser.name : 'Provider'}`,
            description: `${booking.date} at ${booking.timeSlot}${platformFeeSar ? ` (incl. SAR ${platformFeeSar} service fee)` : ''}`,
          },
        },
      }],
      metadata: {
    ...
    return res.json({ success: true, data: { url: session.url, sessionId: session.id }, message: 'Checkout session created' });
  } catch (err) {
    if (req.log && req.log.error) req.log.error({ err }, 'Stripe checkout failed');
    return res.status(502).json({ success: false, message: err.message || 'Failed to create Stripe checkout session.' });
  }
}
...
async function verifyStripeCheckout(req, res) {
  ...
  try {
    ...
    if (session.metadata && session.metadata.bookingId && String(session.metadata.bookingId) !== String(bookingId)) {
      return res.status(400).json({ success: false, message: 'Session does not match this booking.' });
    }
    ...
    const full = await getBookingFull(bookingId);
    return res.json({ success: true, data: { booking: full, paid, paymentStatus: session.payment_status }, message: paid ? 'Payment confirmed' : 'Payment not completed' });
  } catch (err) {
    if (req.log && req.log.error) req.log.error({ err }, 'Stripe verify failed');
    return res.status(502).json({ success: false, message: err.message || 'Failed to verify Stripe payment.' });
  }
}
