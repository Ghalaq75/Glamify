...
async function updateProfile(req, res) {
  ...
  const { name, phone, bio, category, categories, location, specialties, yearsActive, latitude, longitude, coverageRadiusKm } = req.body;
  ...
  if (categories !== undefined || category !== undefined) {
    ...
    providerUpdates.categories = norm.list;
    providerUpdates.category = norm.list[0];
  }
  ...
  return res.json({
    ...
    data: {
      ...
      categories: (Array.isArray(updated.categories) && updated.categories.length)
        ? updated.categories
